...
// Read the HTML file
const html = fs.readFileSync('test-women.html', 'utf8');
...
while ((match = productTilePattern.exec(html)) !== null) {
  ...
  else if (nameLower.includes('shirt') || nameLower.includes('hoody') || nameLower.includes('sweater') || nameLower.includes('zip neck') || nameLower.includes('bottom')) category = 'shirts';
...
const productData = {
  metadata: {
    source: 'Arcteryx Women\'s Products',
...
// Write to JSON file
fs.writeFileSync('src/data/womens-products.json', JSON.stringify(productData, null, 2));

console.log(`Extracted ${products.length} products`);
console.log(`Categories: ${[...new Set(products.map(p => p.category))].join(', ')}`);
console.log(`JSON file written to src/data/womens-products.json`);
